...
# regular expression library and string manipulation
import re
import string

# stream keywords
Pokemon = open('.\pythonpokemon.txt', 'r')
KEYWORDS = Pokemon.read().splitlines()
#print (KEYWORDS)
    ...
    def on_data(self, data):
        ...
        if 'extended_tweet' in tweet:
            ...
            for i in range(len(KEYWORDS)):
                if KEYWORDS[i].lower() in tweet['extended_tweet']['full_text'].lower():
                    client.send_message("/tweet_long", float(i) + 1)
            ...
            if 'extended_tweet' in retweet:
                ...
                for i in range(len(KEYWORDS)):
                    if KEYWORDS[i].lower() in tweet['retweeted_status']['extended_tweet']['full_text'].lower():
                        client.send_message("/retweet_long", float(i) + 1)
                        
                        break
            else:
                #client.send_message("/retweet_short", 'name'+tweet['user']['name']+' -> '+tweet['retweeted_status']['text'])
                for i in range(len(KEYWORDS)):
                    if KEYWORDS[i].lower() in tweet['retweeted_status']['text'].lower():
                        client.send_message("/retweet_short", float(i) + 1)
                       
                        break
        elif 'text' in tweet:
            # client.send_message("/tweet_short", 'name'+tweet['user']['name']+' -> '+tweet['text'])
            for i in range(len(KEYWORDS)):
                if KEYWORDS[i].lower() in tweet['text'].lower():
                    client.send_message("/tweet_short", float(i) + 1)
